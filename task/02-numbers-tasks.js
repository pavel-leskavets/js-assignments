...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    let multiVec = x1*x2 + y1*y2;
    let modul1 = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let modul2 = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));
    return Math.acos(multiVec / modul1 * modul2);
...
function roundToPowerOfTen(num, pow) {
    let rest = Math.pow(10, pow);
    let newNum = num - (num % rest);
    if(num % rest > rest / 2) {
        newNum += rest
    }
    return newNum
